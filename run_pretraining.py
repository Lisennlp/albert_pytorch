...
def main():
    ...
    for epoch in range(args.epochs):
        for idx in range(args.file_num):
            epoch_dataset = PregeneratedDataset(file_id=idx,
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                masked_lm_loss = loss_fct(prediction_scores.view(-1, bert_config.vocab_size),
                                          lm_label_ids.view(-1))
                next_sentence_loss = loss_fct(seq_relationship_score.view(-1, 2), is_next.view(-1))
                loss = masked_lm_loss + next_sentence_loss

                mask_metric(logits=prediction_scores.view(-1, bert_config.vocab_size),
